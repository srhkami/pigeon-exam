  ...
  return (
    <div className='font-bold mb-4 border-l-4 border-l-primary pl-4 flex items-center'>
      ...
      <div role="tablist" className="tabs tabs-border ml-auto">
        <Link role="tab" className={classes(1)} to={EssayPagesForUser.random.url}>{EssayPagesForUser.random.label}</Link>
        <Link role="tab" className={classes(2)} to={EssayPagesForUser.questions.url}>{EssayPagesForUser.questions.label}</Link>
