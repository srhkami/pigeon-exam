...
export default function SelectResultManage() {
  ...
  const {data, pageInfo} = useDataBrowser<PaperRecordData>({url: EXAM_API + '/paper_records/self/'});
    ...
    return (
      <li className="list-row hover:bg-base-200" key={record.id}>
        ...
        <Button style='ghost' shape='circle'
                onClick={() => navi('/paper/record/' + record.id)}>
  ...
  return (
    <DataBrowser
      header={<DataBrowserTitle title='個人測驗紀錄'/>}
